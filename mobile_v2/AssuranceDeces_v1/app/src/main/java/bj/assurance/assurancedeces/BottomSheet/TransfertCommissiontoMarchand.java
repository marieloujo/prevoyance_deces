...
public class TransfertCommissiontoMarchand extends BaseBottomSheet {
            ...
            public void onClick(View v) {
                if (verifData()) {
                    ...
                    //if (contratFormUtils.verifTelephone(matricule.getRawText(), phoneIdentity)) {
                        ...
                        findMarchandByTelephone(phoneIdentity.getSelectedItem().toString().substring(1) + matricule.getRawText());
                ...
                .enqueue(new Callback<JsonObject>() {
                    ...
                    public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                        ...
                        if (response.isSuccessful()) {
                            ...
                            utilisateur = new Gson().fromJson(response.body().getAsJsonObject("success")
                            ...
                            for(int i = 0; i < utilisateur.getUserables().size(); i++) {
                                ...
                                if (isMarchand(utilisateur.getUserables().get(i))) {
                            ...
                            if (isMarchand) {
                                    ...
                                    transfertConfirmation(utilisateur, position);
    ...
    private void transfertConfirmation(final Utilisateur utilisateur, final int i) {
        ...
        showDialog.showQuestionDialog(
                ...
                "Voulez vous vraiment tranferer " + montant.getText().toString() +
                    ...
                    public void onClick(@NotNull LottieAlertDialog lottieAlertDialog) {
                        ...
                        Marchand marchand = new Marchand(utilisateur.getUserables().get(i).getUserableId());



                        compte = new Compte(montant.getText().toString(), marchand);
                        compte.setCompteableId(utilisateur.getId());

                        System.out.println("compte" + new Gson().toJson(marchand));
                        ...
                        if (response.isSuccessful()) {
                            ...
                            switch (string) {
                                ...
                                case "refresh":
                                    ...
                                    String messageTexte = "Vous venez de reçevoir un transfert de " + compte.getMontant() +
                                            " points de la part de " + MarchandActivity.getUtilisateur().getPrenom() +
                                            MarchandActivity.getUtilisateur().getNom() ;

                                    String messageTexte1 = "Vous venez de faire un transfert de " + compte.getMontant() +
                                            " points à " + utilisateur.getPrenom() + " " + utilisateur.getNom() +
                                            " .Votre solde de commission est s'élève actuellement à " + solde.getText().toString() +
                                            " points";


                                    Message message = new Message(messageTexte, null);

                                    new NotificationReader(getContext()).createNotification(message, compte.getCompteableId());


                                    new NotificationReader(getContext()).createNotification(
                                            new Message(messageTexte1, null), MarchandActivity.getUtilisateur().getId());
